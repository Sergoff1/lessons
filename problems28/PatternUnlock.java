    ...
    public static String getUnlockSequence(int N, int [] hits) 
      {
        ...
        for (int i = 0; i < hits.length - 1; i++) {
            int[] firstIndexes = indexOf(dotField, hits[i]);     // firstIndex и secondIndex введены для того, чтобы не загромождать условие в if
            int[] secondIndexes = indexOf(dotField, hits[i+1]);

            boolean isTheSameRow = ( firstIndexes[0] == secondIndexes[0] );
            boolean isTheSameColumn = ( firstIndexes[1] == secondIndexes[1] );
